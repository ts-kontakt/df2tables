...
def create_complex_polars_dataframe(num_rows=100):
    ...
    return pl.DataFrame({
        ...
        'timestamp': [base_date + timedelta(days=random.randint(0, 365)) for _ in range(num_rows)],
        'value': [random.gauss(0, 1) for _ in range(num_rows)],
        'metadata': [{'key': i, 'value': random.random()} if random.random() > 0.2 else None for i in range(num_rows)],
        'duration': [pl.duration(seconds=random.randint(0, 3600)) for _ in range(num_rows)],
    })
...
def render_installed_packages():
    ...
    def get_installed_packages():
        ...
        try:
            ...
            packages = create_random_dataframe(num_rows=100)
        return packages

    header_list = ["name        ", 111, "full package path"]
    df = pd.DataFrame(get_installed_packages(), columns=header_list)
    outfile = get_output_path("pkg_table.html")
    df2t.render(df, to_file=str(outfile))
    print(f"Saved to: {outfile}")
